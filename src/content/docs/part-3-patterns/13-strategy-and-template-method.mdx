...
**Intent:** encapsulate interchangeable algorithms behind one interface and
inject the chosen one, so adding a variant never edits the existing ones.
...
**not when** there are two stable branches (keep the `if`) or a class hierarchy
exists only to hold one method. Use a class strategy only when it carries state.
Runnable: `examples/ch13/concept_strategy.py`.
...
class ConcreteFlow(AbstractFlow):        # supplies steps, keeps the default hook
    def step_one(self): return "one"
    def step_two(self): return "two"

class CustomFlow(AbstractFlow):          # supplies steps and overrides the hook
    def step_one(self): return "1"
    def step_two(self): return "2"
    def hook(self): return "custom"
```

**Pythonic** — for a couple of steps, pass them as functions instead of
subclassing: `run(step_one=..., step_two=...)`. Earn the ABC when steps share
defaults or come in families.

**Reach for it when** the sequence is invariant but steps differ ·
**not when** the whole algorithm varies (that's Strategy) or there's one
subclass. Runnable: `examples/ch13/concept_template_method.py`.
