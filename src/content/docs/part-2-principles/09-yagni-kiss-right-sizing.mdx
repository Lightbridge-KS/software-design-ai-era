...
- **Simple tier** — one module. Functions and dataclasses, stdlib first,
  configuration as parameters. Most scripts, prototypes, and internal tools live
  their whole lives here, correctly.
...
<Tabs>
  <TabItem label="Before (the cathedral, abridged)">
...
```
  ...
  <TabItem label="After (right-sized)">
...
Same behavior. The same tests pass against both — `examples/ch09/test_ch09.py`
runs every assertion through the cathedral and the two functions and demands
identical results. Roughly eighty percent of the code is gone.
...
next chapter opens with exactly that case. Deleting a seam the evidence demands is
...
| "Start at the Simple tier" | One module, functions + dataclasses, stdlib first, config as parameters |
...
Context: this is an internal tool at the **Simple tier** — one module, stdlib
only. Implement the feature below with **no new abstractions, layers, or
...
<TryIt starter="examples/ch09/exercise/settings.py">
...
piece serve? Full instructions in `examples/ch09/exercise/EXERCISE.md`.
...
- A function parameter is the cheapest seam there is, and Python's modules,
  functions, and dataclasses make the Simple tier unusually capable.
