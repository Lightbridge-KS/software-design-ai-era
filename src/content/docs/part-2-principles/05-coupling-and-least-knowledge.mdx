...
caller. When tiers get remodeled next quarter, `is_gold` changes and nothing else
does. The `examples/ch05/` tests prove it the hard way: the after's `free_shipping`
contains neither the word `membership` nor `tier`, so the graph can be rebuilt
beneath it without the caller ever knowing.
...
<TryIt starter="examples/ch05/before.py">
